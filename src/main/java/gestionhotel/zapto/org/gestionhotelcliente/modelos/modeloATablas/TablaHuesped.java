...
public class TablaHuesped {
    ...
    public static ObservableList<TablaHuesped> getTablaBuscadorHuesped(List<Persona> listaPersona) {
        ...
        for (Persona persona : listaPersona) {
            ...
            String discapacitado = (persona.getFisSexoHombre() == 0) ? "Discapacitado" : "No";
    ...
    public TablaHuesped(String numero, String documentoNumero, String nombre, String calle, String codPostal,
     ...
     public Persona getHuesped() {
        ...
        huesped.setCalle(calle);
        huesped.setCategoria(categoria);
        huesped.setFisNacionalidad(nacionalidad);
        huesped.setCiudad(ciudad);
        huesped.setProvincia(provincia);
        huesped.setCodPostal(codigoPostal);
        huesped.setComentario(comentario);
        huesped.setDocumentoNumero(numeroDocumento);
        huesped.setEmail(email);
        huesped.setEsCliente(Byte.parseByte("1"));
        huesped.setEstado(estado);
        if (discapacitado.equals("Discapacitado")) {
            huesped.setFisDiscapacitado(Byte.parseByte("1"));
        } else {
            huesped.setFisDiscapacitado(Byte.MIN_VALUE);
        }
        if (expPasaporte != null&&!expPasaporte.equals("")) {
            
            huesped.setFisExpPasaporte(expPasaporte);
        }
        if (fechaNacimiento != null&&!fechaNacimiento.equals("")) {
            huesped.setFisFechaNacimiento(fechaNacimiento);
        }
        huesped.setFisNacionalidad(nacionalidad);
        huesped.setFisPrimerApellido(primerApellido);
        huesped.setFisSegundoApellido(segundoApellido);
        huesped.setNombre(nombre);
        if (sexoHombre.equals("mujer")) {
            huesped.setFisSexoHombre(Byte.MIN_VALUE);
        } else {
            huesped.setFisSexoHombre(Byte.parseByte("1"));
        }
        huesped.setFisTratamiento(tratamiento);
        huesped.setPaginaWeb(paginaWeb);
        huesped.setPasaporte(pasaporte);
        huesped.setNumero(numero);
        huesped.setComentario(comentario);
